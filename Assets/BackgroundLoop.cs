...
public class BackgroundLoop : MonoBehaviour
{
    ...
    private void repositionChildObjects(GameObject obj) {
        ...
        if(children.Length > 1) {
            GameObject firstChild = children[1].gameObject; // children[0] is the parent object
            GameObject lastChild = children[children.Length-1].gameObject;
            float halfObjectWidth = lastChild.GetComponent<SpriteRenderer>().bounds.extents.x - choke; // extents for half width
    ...
    private void LateUpdate() { // called each frame after Update()
        foreach(GameObject obj in levels) {
